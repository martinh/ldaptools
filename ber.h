...
#define be_numeric	be_union.bv_numeric
	...
	/* State for ber_printf. */
	struct ber_element	*be_ptr;
	struct ber_element	*be_parents[_MAX_SEQ];
	int			 be_level;
};
...
struct ber_element	*ber_printf(struct ber_element *, char *, ...);
int			 ber_scanf(struct ber_element *, char *, ...);
struct ibuf		*ber_write_elements(struct ber_element *);
...
ssize_t			 ber_left(struct ber *ber);
__END_DECLS
