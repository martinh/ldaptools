...
static int	ber_dump_element(struct ibuf *buf, struct ber_element *root);
static void	ber_dump_header(struct ibuf *buf, struct ber_element *root);
static void	ber_putc(struct ibuf *buf, u_char c);
 ...
 */
struct ibuf *
ber_write_elements(struct ber_element *root)
{
	struct ibuf	*buf;
	size_t		 len;
	...
	DPRINTF("write ber element of %zd bytes length", len);

	if ((buf = ibuf_open(len)) == NULL)
		return NULL;

	if (ber_dump_element(buf, root) == -1)
		return NULL;
...
static int
ber_dump_element(struct ibuf *buf, struct ber_element *root)
{
	unsigned long long l;
	int i;
	uint8_t u;

	ber_dump_header(buf, root);
	...
	case BER_TYPE_OBJECT:
		ibuf_add(buf, root->be_val, root->be_len);
		break;
	...
	case BER_TYPE_SET:
		if (root->be_sub && ber_dump_element(buf, root->be_sub) == -1)
			return -1;
		break;
	}
	...
	if (root->be_next == NULL)
		return 0;
	return ber_dump_element(buf, root->be_next);
}

static void
ber_dump_header(struct ibuf *buf, struct ber_element *root)
{
	u_char	id = 0, t, tmp[8];
	unsigned long type;
	size_t size;
	...
	if (root->be_len < BER_TAG_MORE) {
		/* short form */
		ber_putc(buf, root->be_len);
	} else {
		for (t = 0, size = root->be_len; size > 0; size >>= 8)
			tmp[t++] = size & 0xff;

		ber_putc(buf, t | BER_TAG_MORE);
...
static void
ber_putc(struct ibuf *buf, u_char c)
{
	ibuf_add(buf, &c, 1);
}
